...
- **`scripts/selftest.py`** — pipeline self-test: cascade examples, every
  ...
  changing scripts or lists. (Device fonts are prebuilt in `extras/fonts/`;
...
- **`headless/run_book.py` drives** (headless, any model) — the optional
  ...
  ```
  H=ai-tools/graded-reader/headless
...
- **Pinyin display depends on the target device.** On the X3, ruby and
  ...
  a rebuild.
  ```
  Device notes: `extras/readers.md` at the repo root.
...
.venv/bin/python epub-builder/scripts/prepare_cover.py \
    extras/default-covers/graded-default.png --title "愚公移山" \
    --title-config extras/default-covers/graded-default.json \
