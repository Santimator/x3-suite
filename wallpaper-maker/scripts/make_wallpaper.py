...
def fit_panel(img: Image.Image, mode: str = "cover") -> Image.Image:
    """Make the image exactly 528x792.

    'cover' scales to fill and centre-crops the overflow — a wallpaper should
    reach all four edges, and the device will not do this for us. 'contain'
    letterboxes onto white instead, for images whose whole frame matters.
    """
    if mode == "cover":
    ...
    if fitted.size != (PANEL_W, PANEL_H):
...
def convert(src: Path, out_dir: Path, *, fit: str = "cover",
            algorithm: str = "floyd", preview: bool = False) -> Path:
    img = load_grayscale(src)
    img = tone(fit_panel(img, fit))
    levels = dither(img, algorithm)
...
def main() -> int:
    ...
    ap.add_argument("--fit", choices=("cover", "contain"), default="cover",
                    help="cover: fill the panel, crop the overflow (default). "
                         "contain: fit the whole frame, letterbox on white")
    ...
    for src in sources:
        try:
            dest = convert(src, out_dir, fit=args.fit, algorithm=args.dither,
