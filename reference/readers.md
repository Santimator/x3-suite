...
| Font rename | **There is no rename endpoint, and none is needed: the family name *is* the folder name.** `SdCardFontRegistry::scanRoot` takes `family.name` straight from the directory entry, and `parseFilename` only reads `<anything>_<size>.cpfont` for the size — the file prefix is never compared to the folder. So a family is renamed by putting its files under a differently-named folder, and they can keep any prefix. Do **not** rename the directory itself — see the row below. `mkdir` + `move` per file + `delete` the empty original works, and only in the visible `/fonts`: a family under `/.fonts` is unreachable by every route here |
| **WebDAV `MOVE` on a directory destroys it** | **Device-confirmed 2026-08 on an X3, at the cost of a font family.** `MOVE /fonts/NaskhFull` → `Destination: /fonts/RenameTest` answered **201**, and `/fonts/RenameTest` was there and **empty** — the four `.cpfont` files did not come across and were not anywhere else on the card. The handler opens the directory and calls SdFat's `FatFile::rename` (`WebDAVHandler.cpp` handleMove → `HalFile::rename` → `FsFile::rename`), which creates the new entry without the contents following it. It is the only route that will *accept* a directory — `/rename` and `/move` both refuse them — which reads as "the way to rename a folder" and is not. This corrects the earlier source-read claim in this file that a visible folder could be renamed this way. Upstream issue candidate, and the worst one here: it reports success and loses data. *Contributing suspicion, unverified: the family was the selected one, so the font system had its files open for glyph streaming.* |
...
| Folders | `/rename` and `/move` both end in "Only files can be…", and **WebDAV `MOVE` is not the way round it** — it accepts a directory and empties it (row above, device-confirmed 2026-08). **A folder cannot be renamed on this firmware.** What works is doing it by hand: `mkdir` the new one, `move` each file across, `delete` the old one once it is empty — `/delete` takes a directory only when it is already empty, which is the check you want anyway. Nothing recursive: a folder holding folders has to be walked. `/.sleep` is beyond all of it, by either route |
...
- **Reading the sparse-interval rule as optional.** It is the one build-time
  ...
  rules above.
- **Renaming a folder over WebDAV.** It is the only call that will accept a
  directory, it answers 201, and the folder that appears at the other end is
  empty — the files that were in it are gone. That is not a font problem or an
  SD problem; it is the one destructive operation this API offers, and it looks
  exactly like the operation you wanted. Move the files instead. *Cost: one
  font family, 2026-08.*
