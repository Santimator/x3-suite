...
fed over WiFi from the suite's own OPDS server (`tools/opds-server/`) or by SD card.
...
2. **Font:** copy a family folder from `extras/fonts/` to the SD card under
...
Hence the sizes in `extras/fonts/`. Side effects worth knowing: those three
...
**The build** (what produced `extras/fonts/NaskhFull/`; same pinned
...
- **Lists in the picker, reverts to built-in Noto on opening a book** → a
  ...
  check `GET /api/fonts` against `extras/fonts/CHECKSUMS.tsv` — and the
...
own format and folder (see "The sleep screen" below; `tools/wallpaper-maker/` builds
those). For the cover:
...
`tools/wallpaper-maker/` as its sleep screen, from `/.sleep`, mode Custom, quantised
...
rules for an under-size image remain read-from-source.* `tools/wallpaper-maker/` builds these and
pushes them.
...
- **528×792, 4-bpp indexed, 40-byte DIB header, palette on 0/85/170/255,
  ...
  X4-tuned dither never runs. This is what `tools/wallpaper-maker/` does, quantising
...
`tools/wallpaper-maker/scripts/push_wallpaper.py`, and `/rename` was driven by hand on
...
again from `tools/tgbot/` in 2026-08, one file and then three in a batch, with
...
to end from `tools/tgbot/` (2026-08), as was **WebDAV `MOVE` on a directory**, which
...
| Font size | A CJK family is ~24 MB in six files, the largest 6.8 MB; `NaskhFull` is 1.1 MB in four. **How long that takes has never been measured** — earlier notes here claimed "minutes", which was an assumption, and the one person who has actually pushed families reports seconds. The upload timeout is set generously (600 s) because a stalled transfer should fail slowly rather than a slow one fail early, not because anything is known to be slow |
...
browsed a catalog served by `tools/opds-server/` and downloaded books from it; that
...
5. **Verify the bytes afterwards.** Discovery parses filenames and never opens
   ...
   `extras/fonts/CHECKSUMS.tsv` (the Telegram bot does this automatically
   after a push).
...
- **Trusting the copy to the SD card.** Discovery parses *filenames only* — it
  ...
  holds, over WiFi, and match it against `extras/fonts/CHECKSUMS.tsv`:
...
- **Believing the font list after you have changed the fonts folder.** The
  ...
  state of the card. *Hit on 2026-08 through `tools/tgbot/`, and the reason it now
  re-scans after every rename.*

## `.xtc` — the other book format, evaluated and dropped

*Source-confirmed against `lib/Xtc/XtcTypes.h` at 1.5.0, 2026-08.* The reader
opens `.xtc` as well as `.epub`, and the file-transfer server will accept one,
so it looks like an option for getting books onto the card small. It is the
opposite of small.

XTC is **pre-rendered bitmap pages** — the format's own header says "the native
binary ebook format for XTeink X4 e-reader… stores pre-rendered bitmap images
per page", one image per page at **480×800**, which is the X4's panel and not
the X3's 528×792. Page data is **uncompressed** (`compression: 0`), and the
header spells out the arithmetic: 1-bit pages are `((width+7)/8)*height` =
**48 KB each**, 2-bit greyscale twice that. A 300-page book is **~14 MB at
1 bit and ~29 MB at 2 bits** — against ~200 KB for the same book as a slimmed
EPUB, and more than the fat EPUB it would replace.

Size was the only reason to want it, and it loses on size by two orders of
magnitude. It also gives up everything that makes this device pleasant: baked
pages do not reflow, so font family, size, line spacing and margins are all
inert — no WenZilla, no CrimKai, and no `NaskhFull`, because there is no text
left for the shaper to shape. No search either. A converter exists (EPUB2XTC,
by Rafal-P-Mazur, which this parser is based on); it is not worth using here.
