...
`extras/readers.md`; this file is that table expressed as code.
...
written, and proved against a real device) and `tools/tgbot/`. That is why it lives
...
# A font family is ~24 MB in six files, the largest of them 6.8 MB. How long
# that actually takes over WiFi has never been measured — reports say seconds,
# and the only claim of "minutes" in this repo was an assumption, now removed.
# The timeout stays generous anyway: it costs nothing when transfers are quick,
# and a bad link should fail slowly rather than a good one fail early.
FONT_TIMEOUT = 600
