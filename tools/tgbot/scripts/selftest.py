...
"""The gate: does the bot refuse what it must refuse, and forget nothing?
    ...
    python3 tools/tgbot/scripts/selftest.py
...
def check_entry_point() -> None:
    ...
    so it passed green while `python3 tools/tgbot/scripts/bot.py` died on an
...
def check_secrets_outside(tmp: Path) -> None:
    ...
    inside_cfg.write_text(json.dumps({"secrets_dir": str(conf.REPO_ROOT / "tools" / "tgbot"),
                                      "telegram": {"user_id": 9}}))
